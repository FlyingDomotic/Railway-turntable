var ctx = 0;                                                        // Canvas context
var canvas = 0;                                                     // Canvas object
var lastAngle = 0;                                                  // Last received angle
var trackCount = 0;                                                 // Count of tracks used
var trackAngle = [-1.0];                                            // List of used tracks angles
var imageOffsetAngle = 0;                                           // Image offset angle (degrees)
var degreesPerStep = 0;                                             // Stepper degrees per step
var stepperReduction = 0;                                           // Stepper reduction
var microStepsPerStep = 0;                                          // Stepper micro steps per step
var radius = 0;                                                     // Radius of circles (in percentage of image)
var startEventsPending = false;                                     // Do we have a start event pending ?
var startEventsDelay = 0;                                           // Event delay before trying to reconnect
var inSetup = false;                                                // Are we in setup.htm?
...
function drawCircles() {
    var w = 32;                                                     // Circles width
    var imgWidth = ctx.canvas.width;                                // Image desired width & height
    var imgHeight = ctx.canvas.height
    ctx.save();                                                     // Save context
    for (let i = 1; i <= trackCount; i++) {                         // For all used tracks
        var angle = parseFloat(trackAngle[i]);                      // Get angle of track
        // Compute x & y giving angle and image size
        var x = (radius / 100.0 * Math.cos(angle * Math.PI / 180.0) * imgWidth) + (imgWidth / 2.0);
...
function startEvents(){
    ...
    es.addEventListener('angle', function(e) {	                    // Executed when receiving an "angle" event
        console.log("Angle: "+ e.data);                             // Log
    ...
    es.addEventListener('settings', function(e) {	                // Executed wurh receiving a "settings" event
...
function turn(angle) {
    let img2 = new Image();                                         // Create new image
    angle -= imageOffsetAngle;                                      // Fix angle using imageOffset
    img2.onload = function () {                                     // Executed when image is loaded
        ctx.save();                                                 // Save context
    ...
    img2.src = "plaque.png";                                        // Image to load
}
